...
namespace Invoice.WebApi {
    public class Startup {
        ...
        public void ConfigureServices(IServiceCollection services) {
            ...
            services.AddXaf<InvoiceWebApiApplication>(Configuration);
            services.AddXafSecurity(options => {
                options.RoleType = typeof(PermissionPolicyRole);
                // ApplicationUser descends from PermissionPolicyUser and supports the OAuth authentication. For more information, refer to the following topic: https://docs.devexpress.com/eXpressAppFramework/402197
                // If your application uses PermissionPolicyUser or a custom user type, set the UserType property as follows:
                options.UserType = typeof(Invoice.Module.BusinessObjects.ApplicationUser);
                // ApplicationUserLoginInfo is only necessary for applications that use the ApplicationUser user type.
                // If you use PermissionPolicyUser or a custom user type, comment out the following line:
                options.UserLoginInfoType = typeof(Invoice.Module.BusinessObjects.ApplicationUserLoginInfo);
                ...
                options.Events.OnAuthenticated = (externalAuthenticationContext) => {
                    ...
                    externalAuthenticationContext.Principal.Identity.AuthenticationType != SecurityDefaults.WindowsAuthentication && !(externalAuthenticationContext.Principal is WindowsPrincipal)) {
                        ...
                        object CreateApplicationUser(string userName, string providerUserId) {
                            if(objectSpace.FirstOrDefault<Invoice.Module.BusinessObjects.ApplicationUser>(user => user.UserName == userName) != null) {
                                throw new ArgumentException($"The login with '{userName}' name was already registered within the system");
                            }
                            var user = objectSpace.CreateObject<Invoice.Module.BusinessObjects.ApplicationUser>();
                        ...
                        ISecurityUserLoginInfo FindUserLoginInfo(string loginProviderName, string providerUserId) {
                            return objectSpace.FirstOrDefault<Invoice.Module.BusinessObjects.ApplicationUserLoginInfo>(userLoginInfo =>
            ...
            services.AddSwaggerGen(c => {
                ...
                c.SwaggerDoc("v1", new OpenApiInfo {
                    Title = "Invoice API",
                    Version = "v1",
                    Description = @"Use AddXafWebApi(options) in the Invoice.WebApi\Startup.cs file to make Business Objects available in the Web API."
                });
                    ...
                    Flows = new OpenApiOAuthFlows() {
                        AuthorizationCode = new OpenApiOAuthFlow() {
                            ...
                            Scopes = new Dictionary<string, string> {
                                ...
                                { @"[Enter the scope name in the Invoice.WebApi\Startup.cs file]", @"[Enter the scope description in the Invoice.WebApi\Startup.cs file]" }
                            }
                        }
                    }
                });
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env) {
            if(env.IsDevelopment()) {
                ...
                app.UseSwagger();
                app.UseSwaggerUI(c => {
                    c.SwaggerEndpoint("/swagger/v1/swagger.json", "Invoice WebApi v1");
